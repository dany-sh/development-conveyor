...
15. A validated model-produced candidate is finalized deterministically into one authoritative accepted commit whose only parent is the planned milestone start and whose tree adds only the queue, feature specification, feature catalog, current status, and run log metadata. The queue uses `accepted_commit: SELF`.
16. No feature reaches `integration_ready` until the accepted commit itself passes exact parent/ref, implementation-tree equivalence, complete acceptance metadata, and the unchanged immutable two-ref integration validator.
...
- Normal finalization and protected accepted-commit recovery share one Git-plumbing finalizer. It records candidate and finalized commit identities separately, reconstructs a direct child of the milestone base instead of adding a metadata child, limits candidate-to-finalized differences to the five established metadata paths, moves the feature ref atomically, and validates the committed `SELF` metadata before terminal projection.
