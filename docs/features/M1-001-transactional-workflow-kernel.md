...
The focused F005 recovery repair requires two independent immutable refs after a terminal pre-mutation integration failure. The milestone ref must still equal the failed starting snapshot; the accepted feature ref must self-identify in its committed queue snapshot with `accepted_commit: SELF`, the exact branch and integration base, pending integration, and complete acceptance evidence. Its HEAD must be a non-merge commit with the exact milestone HEAD as its sole parent, proving one direct feature commit. The live queue and failed report cannot select the accepted commit. A verified candidate starts a fresh integration transaction only after both refs are revalidated under the launch reservation; the old session remains non-resumable.
