...
One compatible terminal marker can be normalized before it is classified as invalid, but only under exact controller corroboration. The queue-reconciliation workflow may map `repository` to `repository_identity`, `result` to `classification`, and supply the invoked `project_id` and `workflow_type`; it must still carry matching transaction, run, session, branch, HEAD, and repository identity. Conflicting canonical and alternate fields are rejected. `RECONCILED_READY_WORK` always maps to kernel state `feature_ready`; legacy `feature_preparation` or `feature_preparing` values are treated only as compatible spellings of that canonical transition.
...
`scripts/conveyor recover-planning` is the bounded recovery interface for a previously recorded dirty transaction and requires exactly one of `--dry-run` or `--apply`. Dry-run requires the expected starting HEAD, diff fingerprint, session ID, and every exact changed path, runs all validators, and writes nothing. `--apply` revalidates the same evidence under the planning lease, creates one planning-only commit, persists controller evidence, makes that commit the selected feature's starting commit, releases the lease, and stops at `feature_ready`. A retained compatible terminal result also preserves historical collaboration-tool attempts as diagnostic recovery evidence without relaunching them. An evidence mismatch preserves all files and refs, writes a planning recovery gate only on apply, does not invalidate completed integration, and launches neither Feature Factory nor Milestone Integrator. A successful recovery cannot be applied again; an interruption after Git commit but before controller persistence is recovered from exact parent, subject, paths, fingerprint, and session evidence.
