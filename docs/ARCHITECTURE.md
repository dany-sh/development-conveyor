...
controller derives the requested skill allowlist from the workflow, disables
every other prompt-visible skill by exact `SKILL.md` path, disables unrelated
plugins and the complete MCP map through per-invocation configuration, and
reruns pinned-CLI `debug prompt-input`. Requested and effective allowlists are
recorded separately. Any missing required skill, remaining unrelated skill,
configuration failure, truncation concern, or unrelated MCP authentication
attempt returns `capability_isolation_unsupported` before model launch. No
global capability installation or configuration is mutated.

Feature capability selection distinguishes the required workflow core from
optional domain skills. Source-language discovery never makes a plugin skill
mandatory. A selected feature may require a build-macos-apps skill by naming
its exact capability identifier in the feature contract; that declaration is
then included in the requested allowlist and must pass the same pinned-CLI
proof. Missing explicit capabilities still fail closed.

Recovery routing is terminal-owner scoped. Historical planning and integration
failures cannot intercept a later feature, integration, gate, or unrelated
recovery failure merely because the projection shares a generic failed state.
The latest projected terminal transaction must own the recovery route, except
for an independently authenticated supported recovery supersession.

Run-scoped capability-isolation failures reuse the zero-session feature
prelaunch recovery boundary only when the latest feature transaction and its
launch-failure report prove empty argv, absent session and launched-model
identity, no terminal marker, the exact `feature_cycle` action and repository,
...
Retained-result recovery recognizes three exact feature-session event shapes:
the legacy topology, the authenticated-session checkpoint topology, and the
current prepared-branch plus authenticated-session checkpoint topology. When
execution follows capability-prelaunch recovery, continuity additionally
requires the exact failed zero-session transaction, its authenticated
run-scoped isolation report, the complete deterministic recovery transaction,
and matching preparation, execution, branch, HEAD, queue, and diff evidence.
Only an evidence-authenticated plan may bypass the raw structured-output human
gate during ordinary resume.
...
Feature execution profile selection is metadata-owned and independent from context discovery or verification planning. `config/execution-profiles.yaml` validates the eight named model/reasoning pairs, workflow fallbacks, and controller-owned reconciled feature policies. The resolution order is deterministic zero-model work, explicit run override, selected feature policy, historical workflow fallback, then recorded evidence-based escalation. The resolved pair is bound to Codex argv and policy source; parent and child counts are enforced at the launcher boundary. `application_feature_implementation` and the generic application fallback resolve to exact `gpt-5.6-terra` with medium reasoning, one parent, and zero children unless authenticated feature metadata selects another exact profile. Queue reconciliation and context preparation retain Luna; controller architecture and generic controller implementation retain Sol/medium; ambiguous durable authority, recovery, integration, or dispatch retains Sol/high. Xhigh requires a concrete failed or unresolved high-reasoning record and is unavailable on the first attempt. Newly promoted ready features must carry matching queue and specification `execution_policy` metadata. Ordinary reconciliation preserves explicit valid policy and otherwise plans the canonical `application_feature` fallback, but validates feature identity, queue/dependency evidence, inventory, authorized paths, diff integrity, and document semantics before materializing that deterministic metadata. The normalization remains ledger-bound to the session's original changed paths, exact normalization paths, and final diff fingerprint; invalid, contradictory, unavailable, silently substituted, or post-validation-changed policy fails closed. Existing ready entries may continue through safe optional-field defaults without mass editing.

Planning finalization recovery derives protocol topology after removing
`CheckpointRecorded` events. Checkpoints are transparent audit metadata
regardless of name or count, but every checkpoint must retain the same
transaction/workflow, run/session, branch/HEAD, lease ownership, mutation
authority, authorized path sets, retained fingerprint, and model/child
session authority. A normalization checkpoint may bind distinct source and
final path sets only when the source paths plus authorized normalization paths
exactly cover the retained result and its final fingerprint. Missing
checkpoints are valid; conflicting checkpoint metadata fails closed.

Context construction records file count, approximate bytes, included paths,
inclusion reasons, and excluded categories. Feature execution includes the
selected and dependency specifications, repository instructions, adapter,
direct architecture contracts, relevant source and tests, execution profile,
and terminal contract. Queue reconciliation deterministically limits
candidate specifications. Route selection never loads skill bodies.

Every model prompt ends with one controller-owned compact JSON output
contract. Exact workflow naming is validated after the sole documented
`feature_cycle` to `feature_execution` alias. Successful command output is
reduced to command, exit code, duration, output hash, and bounded summary;
errors, safety findings, changed paths, validation failures, blockers, commit
identity, and recovery instructions remain mandatory.
