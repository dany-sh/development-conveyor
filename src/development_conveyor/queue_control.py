...
def queue_report(
    ...
    *,
    ...
    current = (runtime or {}).get("current_feature")
    selected = (runtime or {}).get("selected_next_feature")
    if selected is None:
        selected = (runtime or {}).get("selected_feature")
...
def prioritize(
    ...
    *,
    ...
    if any(inspector.git_operation_state().values()):
        raise ConveyorError("prioritize is unavailable during an unfinished Git operation")
    if inspector.current_branch != project.milestone_branch:
        raise ConveyorError("prioritize requires the configured milestone branch")
    ...
    if changed - {relative_queue} or untracked:
        raise ConveyorError("prioritize requires a clean repository except for the queue file")
    ...
    if inspect_repository_writer_lock(writer_path, project.repository).exists:
        raise ConveyorError("prioritize is unavailable while another writer lease exists")
    ...
    relative = queue.feature(relative_id)
    if (
        feature is None
        or relative is None
        or feature.get("milestone") != project.active_milestone
        or relative.get("milestone") != project.active_milestone
    ):
        raise QueueError("prioritize features must both belong to the active milestone")
    ...
    lease_run = f"prioritize-{uuid.uuid4()}"
    ...
    try:
        ...
        if set(inspector.tracked_changed_paths()) != {relative_queue}:
            raise ConveyorError("prioritize changed a path other than FEATURE_QUEUE.yaml")
    except Exception:
