...
Use the named milestone integration workflow in read-only preparation mode. Verify accepted commit `{accepted_commit}`, the milestone branch, ancestry, and configured validation plan, but do not replace the accepted commit with a queue sentinel and do not switch branches, cherry-pick, stage, commit, or write runtime evidence. The controller kernel performs the exact-once cherry-pick and owns integration evidence after accepting your typed result.
