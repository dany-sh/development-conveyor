...
Post-integration commands are classified as `required_validation`, `required_evidence_finalization`, `optional_diagnostic`, `status_observation`, or `unsupported_command`. Required failures remain authoritative. An unconfigured missing repository-local diagnostic is retained with its exit status, classification, and warning-only effect; it cannot override corroborated durable success.

Durable integration success takes precedence over a stale controller failure only when the terminal `INTEGRATED` result has a compatible zero process exit and independent evidence agrees: the report schema, project, run, action, resolved working directory, and nonblank exact session identity; unique accepted and integrated commits; patch equivalence; integrated/passed queue state; milestone membership; passing required integration commands; complete blocker-free runtime evidence with `clean_worktree: true`; the exact direct-parent integration/evidence/model-evidence commit chain; the final validated milestone HEAD; clean current Git state; no active Git operation; no writer lease; and a completed repository cycle. Explicit integration-fix commits must appear in the same order in queue and runtime records, extend the integration chain before validation, and have command evidence whose producer record names the final fix commit as its validated tree; stale pre-fix commands cannot authorize success. Post-validation commits remain metadata-only. Finalization preserves the prior successful checkpoint and history, clears obsolete failure fields, records the final milestone HEAD as the effective `last_validated_commit` without rewriting application queue metadata, and routes the controller through a legal transition to `queue_reconciliation`.
...
A proposed feature that has satisfied dependencies remains planning work and routes to `queue_reconciliation`; it is never launched as ready work. A future `requires_human_decision` item becomes a current queue gate only after its own dependencies are complete. In `one_feature` mode, durable integration finalization stops without relaunching Feature Factory or Milestone Integrator; milestone mode may reconcile proposed work in a later controller step. Once finalized, later planning-only documentation commits may advance the milestone branch without invalidating historical integration evidence, but any later production or test commit requires fresh validation.
...
| --- | --- | --- | --- | --- |
| `case-manager` | `${HOME}/Developer/conan-case-manager` | `P0` | P0-001 and P0-002 are integrated; controller finalization is recoverable from durable evidence | Reconcile proposed P0-003 planning without launching it as ready work. |
